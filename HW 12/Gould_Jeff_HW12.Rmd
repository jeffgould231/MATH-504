...
The matrix determinant lemma ([reference](https://en.wikipedia.org/wiki/Matrix_determinant_lemma)) states that if $A$ is an $n \times n$ invertible matrix, and $U$ and $V$ are $n \times m$ matrices, the $\det(A + UV^T) = \det(I_m +V^TA^{-1}U)\det(A)$. Applying to our data, we get $\det(\Omega + B^TB) = \det(I_{259} + B\Omega^{-1}B^T)\det(\Omega) \rightarrow \det(\rho\Omega + B^TB) = \det(I_{259} + B\rho^{-1}\Omega^{-1}B^T)\det(\rho\Omega)$. Since we know that $\det(\Omega)$ is very large (`R` outputs it as $\infty$), we simply need to show that $\det(I_{259} + B\Omega^{-1}B^T) \neq 0$:
